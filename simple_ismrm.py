from src.tracker import SeedBasedTracker
...
class ModelTracker(SeedBasedTracker):
    ...
    def _trackWithModel(self, no_iterations=200, batch_size=65536):
        ...
        for idx in range(0, len(seeds), batch_size):
            model.reset()
            idx_range = range(idx, min(idx+batchSize, len(seeds)))    
            idx_range_back = range(idx + len(seeds), min(idx+batchSize, len(seeds)) + len(seeds))

            #First prediction
            points = streamlines[idx_range, 0]
            points_w_grid = points[:, None, None, None, :] + grid
            dwi = data_container.get_interpolated_dwi(points_w_grid).reshape(len(points), grid.size, -1)
            print(dwi.shape)
