...
> ```python 00_generate_paddedData.py [options]```
...
> ```python 01_trainModel.py [options]```
...
The test tracking with the ISMRM 2015 data set is realized by this script. The possibility to use FA stopping in addition to the model-internal stop prediction should be emphasized, if the ```-fa``` option is used.

> ```python 03_ismrm2015.py [-tr/-fa <value>] [options]```


The options are the following:

| option                    | description                                                    | value   | default      | optional           |
| ------------------------- | -------------------------------------------------------------- | ------- | ------------ | ------------------ | 
| **-c** <value>            | the Case ID, used for save file's name.                        | string  | "ISMRM[...]" | :heavy_check_mark: |
| **-n** <value>            | the number of steps done while tracking                        | integer | 200          | :heavy_check_mark: |
| **-b** <value>            | the B-value of the DWI Data                                    | integer | 1000         | :heavy_check_mark: |
| **-sw** <value>           | the step width to use while tracking                           | float   | 1.0          | :heavy_check_mark: |
| **-tr** <value>           | the threshold to use for Tracking stop. Between 0 and 1        | float   | 0.5          | :heavy_check_mark: |
| **-mil** <value>          | the minimum streamline length in mm                            | integer | 20           | :heavy_check_mark: |
| **-mal** <value>          | the maximum streamline length in mm                            | integer | 200          | :heavy_check_mark: |
| **-fa** <value>           | the FA-threshold to use for tracking. Also toggles FA-tracking | float   | disabled     | :heavy_check_mark: |
| **-sh** <value>           | order of spherical harmonics if used                           | integer | 4            | :heavy_check_mark: |
| **-spc** <value>          | the grid spacing in pixels/IJK                                 | float   | 1.0          | :heavy_check_mark: |
| **--denoise**             | activate Denoising of dataset                                  |         |              | :heavy_check_mark: |
| **--reslice**             | reslice dataset to 1.25mm³ instead                             |         |              | :heavy_check_mark: |
| **--useMLP**              | use linear instead of recurrent model                          |         |              | :heavy_check_mark: |
| **--rotateData**          | rotate the data instead of constant rotation                   |         |              | :heavy_check_mark: |
| **--dontRotateGradients** | don't rotate the gradients as per default                      |         |              | :heavy_check_mark: |
| **--faMask**              | use faMask for seed point choice                               |         |              | :heavy_check_mark: |

The results are then saved into the ```results_tracking/``` folder.