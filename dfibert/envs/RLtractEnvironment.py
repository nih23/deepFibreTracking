...
class RLtractEnvironment(gym.Env):
    def __init__(self, device, seeds, stepWidth=0.8, action_space=20, dataset='100307', grid_dim=[3, 3, 3],
        ...
        self.device = device
        self.dataset = DataPreprocessor().normalize().crop(b_val).fa_estimate().get_hcp(f"data/HCP/{dataset}/")
        # normalize HCP data
    ...
    def changeReferenceStreamlinesFile(self, pReferenceStreamlines):
        ...
        if (self.tracking_in_RAS):
            self.tracked_streamlines_torch = [torch.FloatTensor(self.dataset.to_ijk(x)).to(self.device) for x in
                                              self.tracked_streamlines]
        else:
    ...
    def _correct_direction(self, action_vector):
        ...
        if self.stepCounter <= 1:  # if no past states
            last_direction = self.referenceStreamline_ijk[1] - self.referenceStreamline_ijk[
                0]  # get the direction in which the reference steamline is going
