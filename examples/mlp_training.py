...
class ModelMLP(nn.Module):  # the NN as PyTorch module, this is best practise to keep the structure modular and simple.
    ...
    def forward(self,
                x):  # default function for propagating data through the network, do not change name or arguments.
        # called with mlp_model(data)
    ...
    def _feed_model(self, generator,
                    validation=False):  # prepares data in mini batches and passes them through the network
        ...
        for batch, (dwi, next_dir) in enumerate(generator):
            ...
            whole_loss += next_dir.shape[
                              0] * loss.item()  # necessary for loss calculation, loss.item() is mean over batch
...
def main():
    ...
    training_set, validation_set = random_split(
        dataset)  # randomly splits the dataset into training and validation. Default 90% Training

    model = ModelMLP(hidden_sizes=[512, 512, 512], activation_function=nn.ReLU(), dropout=0.05, input_size=2700).to(
        device)  # Initialize the model

    params = {'batch_size': 2048, 'num_workers': 0,
              'shuffle': True}  # !!!! NUM WORKERS > 0 does not work with caching yet !!!
