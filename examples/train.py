...
def train(path, max_steps=3000000, replay_memory_size=20000, start_learning=10000, eps_annealing_steps=100000, agent_history_length=1, evaluate_every=20000, eval_runs=5, network_update_every=10000, max_episode_length=200, learning_rate=0.0000625):
    ...
    print("Init agent")
    state = env.reset()
    agent = Agent(n_actions=n_actions, inp_size=state.getValue().shape, device=device, hidden=512, agent_history_length=agent_history_length, memory_size=replay_memory_size, learning_rate=learning_rate, batch_size=32, gamma=0.5)
    ...
    ######## fill memory begins here
        while (epoch_step < evaluate_every) or (step_counter < start_learning):
            ...
            while not terminal:
                ...
                action = action_scheduler.get_action(step_counter, state.getValue().float().to(device).unsqueeze(0))
                ...
                agent.replay_memory.add_experience(action=action,
                                    state=state.getValue().float().numpy(),
                                    reward=reward,
                                    new_state=next_state.getValue().float().numpy(),
                                    terminal=terminal)
            ...
            if len(eps_rewards) % 10 == 0:
                ...
                print("[{}] {}, {}, current eps {}, avg steps {}".format(len(eps_rewards), step_counter, np.mean(eps_rewards[-100:]), action_scheduler.eps_current, np.mean(eps_steps)) )
    ...
    ########## evaluation starting here
        ...
        for _ in range(eval_runs):
            ...
            while eval_steps < max_episode_length:
                action = action_scheduler.get_action(step_counter, state.getValue().float().to(device).unsqueeze(0), evaluation=True)
